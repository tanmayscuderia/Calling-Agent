...
| **CI** | `.github/workflows/ci.yml` — backend typecheck + 337 unit tests (fully mocked, no secrets) + frontend build. **Manual trigger only** (`workflow_dispatch` — Actions tab or `gh workflow run ci.yml`); no push/PR runs. |
